...
Job.prototype.takeLock = function(token, renew){
  console.log("[Job.prototype.takeLock] start! token:", token)
  ...
  return this.queue.client.setAsync.apply(this.queue.client, args).then(function(result){
    console.log("[Job.prototype.takeLock] success! token:", token)
    return result === 'OK';
  });
};
...
Job.prototype.renewLock = function(token){
  console.log("[Job.prototype.renewLock] token:", token)
...
Job.prototype.releaseLock = function(token){
  console.log("[Job.prototype.releaseLock] token:", token)
